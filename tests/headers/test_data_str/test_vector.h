...
TEST(VectorPointer, CreationAndInsertion) {
  ...
  *cur_item = 3;
  ...
  delete[] cur_item;  // must explicitly delete the current item
  ...
  *cur_item = 4;
  ...
  // explicitly delete all of vec2's elements before assigning them
  for (uint32_t i = 0; i < vec2->size(); i ++) {
    cur_item = *vec2->at(i);
    delete[] cur_item;
  }
  ...
  *vec2 = *vec1;  // should force a resize of vec2's capacity to 4
  ...
  // explicitly delete all of vec1's elements before destroying the vector

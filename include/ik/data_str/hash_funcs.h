...
#include <string>
#include "ik/math/math_types.h"  // for uint
#include "ik/string_util/macros.h"
...
#define CONSTANT_HASH(size, str) ik::data_str::HashString(size, str)
#else
#define CONSTANT_HASH(size, str) \
  (ik::data_str::StringHash(str).m_hash) % (size - 1)
#endif

#define DYNAMIC_HASH(size, str) ik::data_str::HashString(size, str)
